...
public interface UserMapper extends BaseMapper<User> {
    ...
    @Select("select user.*,area.`name` as area,department.`name` as department from user,area,department where user.`area_id`=area.`id` and user.`department_id`=department.`id`")
